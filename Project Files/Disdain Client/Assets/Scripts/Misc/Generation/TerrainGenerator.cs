...
public class TerrainGenerator : MonoBehaviour
{
	...
	{
		Vector2 chunkLoc = new Vector2(Mathf.RoundToInt(obj.transform.position.x / instance.meshWorldSize), Mathf.RoundToInt(obj.transform.position.z / instance.meshWorldSize));
		if (!instance.terrainChunkDictionary.ContainsKey(chunkLoc)) { return false; }

		if (childType == ChildType.Prop)
		{
			obj.transform.parent = instance.terrainChunkDictionary[chunkLoc].props.transform;
		}
		else if (childType == ChildType.Structure)
		{
			obj.transform.parent = instance.terrainChunkDictionary[chunkLoc].structures.transform;
		}
		else if (childType == ChildType.Item)
		{
			obj.transform.parent = instance.terrainChunkDictionary[chunkLoc].items.transform;
		}
		else if (childType == ChildType.Vehicle)
		{
			obj.transform.parent = instance.terrainChunkDictionary[chunkLoc].vehicles.transform;
		}
		return false;
	}
