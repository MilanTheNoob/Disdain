...
public class PropsGeneration : MonoBehaviour
{
    ...
    public void Generate(TerrainChunk chunk)
    {
        ...
        if (SavingManager.SaveFile.Chunks.ContainsKey(new Vector2(chunk.coord.x, chunk.coord.y)))
        {
            ...
            List<PropData> Vehicles = SavingManager.SaveFile.Chunks[chunk.coord].Vehicles;
            ...
            {
                for (int j = 0; j < PropSettings.Cars.Length; j++)
                {
                    if (PropSettings.Cars[j].name == Vehicles[i].Name)
                    {
                        GameObject vehicle = Instantiate(PropSettings.Cars[j]);
