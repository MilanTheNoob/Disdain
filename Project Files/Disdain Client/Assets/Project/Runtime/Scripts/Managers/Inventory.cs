...
public class Inventory : MonoBehaviour
{
    ...
    private void Start()
    {
        DataManager.SaveFileReceiveCallback += OnGetSaveFile;
    }

    void OnGetSaveFile()
    {
        for (int i = 0; i < DataManager.SaveFile.Inventory.Count; i++) { items.Add(Resources.Load<ItemSettings>("Interactable Items/" + DataManager.SaveFile.Inventory[i])); }
    }
    ...
    public bool Add(ItemSettings itemSettings)
    {
        if (items.Count <= 28)
        {
            ...
            DataManager.SaveFile.Inventory.Add(itemSettings.name);
    ...
    public void Remove(ItemSettings itemSettings)
    {
        ...
        if (DataManager.GameState == GameStateEnum.Singleplayer)
        {
            ...
            if (DataManager.GameState == GameStateEnum.Singleplayer) { DataManager.SaveFile.Inventory.Remove(itemSettings.name); }
            onItemChangedCallback.Invoke();
        }
        else if (DataManager.GameState == GameStateEnum.Multiplayer)
        {
    ...
    public void Destroy(ItemSettings itemSettings)
    {
        if (DataManager.GameState == GameStateEnum.Singleplayer)
        {
            DataManager.SaveFile.Inventory.Remove(itemSettings.name);
