...
public class Generation : MonoBehaviour
{
	...
	{
		if (!chunk.PropsGenerated) { return; }
        ...
        {
			for (int i = 0; i < chunk.PropParents[b].transform.childCount; i++)
			{
				Transform prop = chunk.PropParents[b].transform.GetChild(i);
				string[] nValues = chunk.PropParents[b].transform.name.Split(',');

				SaveFileClass.PropClass propData = new SaveFileClass.PropClass
				{
					ID = int.Parse(nValues[0]),
					Scale = prop.localScale,
					Pos = prop.position,
					Euler = prop.eulerAngles
				};
				chunkData.PropTypes[b].Props.Add(propData);
			}
		}

		if (DataManager.SaveFile.FindChunk(chunkData.Coords) == -1)
		{
			DataManager.SaveFile.Chunks.Add(chunkData);
		}
		else
		{
			DataManager.SaveFile.Chunks[DataManager.SaveFile.FindChunk(chunkData.Coords)] = chunkData;
		}
	}
    ...
    public static MeshData GenerateTerrainMesh(float[,] heightMap, MeshSettingsStruct meshSettings, int levelOfDetail)
	{
