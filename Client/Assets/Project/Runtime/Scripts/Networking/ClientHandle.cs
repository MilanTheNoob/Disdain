...
public class ClientHandle : MonoBehaviour
{
    ...
    public static void OnConnect(Packet packet)
    {
        ...
        DataManager.instance.StartCoroutine(IOnConnect());
    }

    static IEnumerator IOnConnect()
    {
        while (SceneManager.GetActiveScene().buildIndex != 1) { yield return new WaitForSeconds(0.5f); }
    ...
    public static void LauncherData(Packet packet)
    {
        ...
        launcherData.UpdatePatchNotes = new LauncherDataClass.UpdatePatchNotesClass[packet.ReadInt(false)];
        for (int i = 0; i < packet.ReadInt(); i++)
        {
            ...
            patchNote.Paragraphs = new string[packet.ReadInt(false)];
            for (int j = 0; j < packet.ReadInt(); j++) { patchNote.Paragraphs[j] = packet.ReadString(); }

            launcherData.UpdatePatchNotes[i] = patchNote;
        }

        LauncherManager.SetLauncherData(launcherData);
        DataUIManager.FinishInit();
    }
