...
{
    #region Generic Funcs

    public static void SendString(int id, string s) { Packet packet = new Packet(id); packet.Write(s); packet.Send(); }
    public static void SendInt(int id, int i) { Packet packet = new Packet(id); packet.Write(i); packet.Send(); }
    public static void SendEmpty(int id) { new Packet(id).Send(); }

    #endregion

    #region Init

    public static void OnConnect() { new Packet(0).Send(); }
    ...
    public static void SkinSend()
    {
        Packet packet = new Packet(9);

        packet.Write(LoginManager.player_gender);
        packet.Write(SkinManager.CC.materialActiveIndex);

        packet.Write(SkinManager.CC.headActiveIndex);
        packet.Write(SkinManager.CC.hairActiveIndex);

        packet.Write(SkinManager.CC.ClothesID[ClothesType.Hat]);
        packet.Write(SkinManager.CC.ClothesID[ClothesType.TShirt]);
        packet.Write(SkinManager.CC.ClothesID[ClothesType.Pants]);
        packet.Write(SkinManager.CC.ClothesID[ClothesType.Shoes]);
        packet.Write(SkinManager.CC.ClothesID[ClothesType.Accessory]);

        packet.Write(LoginManager.Fatness);
        packet.Write(LoginManager.Muscles);
        packet.Write(LoginManager.Slimness);
        packet.Write(LoginManager.Thinness);
        packet.Write(LoginManager.Breasts);
    ...
    public static void LauncherData()
    {
        Packet packet = new Packet(1);
        packet.Write(DataManager.MainVersion);
        packet.Write(DataManager.ContentVersion);
        packet.Write(DataManager.BugFixVersion);
