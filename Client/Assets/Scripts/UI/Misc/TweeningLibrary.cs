...
public class TweeningLibrary : MonoBehaviour
{
    ...
    public static void FadeOut(GameObject g, float d, bool disable = true) { if (tweening.Contains(g.GetInstanceID()) || instance == null || g == null) { return; } instance.StartCoroutine(FadeCanvasGroup(g, 1, 0, false, disable, d)); }
    public static void FadeIn(GameObject g, float d) { if (tweening.Contains(g.GetInstanceID()) || instance == null || g == null) { return; } g.SetActive(true); instance.StartCoroutine(FadeCanvasGroup(g, 0, 1, true, false, d)); }

    // The fading coroutine used to affect a UI's alpha
    static IEnumerator FadeCanvasGroup(GameObject g, float start, float end, bool s, bool disable, float lerpTime = 1)
    {
        ...
        while (true)
        {
            ...
            if (percentageComplete >= 1)
            {
                ...
                if (disable) { g.SetActive(s); cg.alpha = 1; }
